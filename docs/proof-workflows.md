...
For runtime-generation prompts, those compact explain packs now also preserve NestJS/BullMQ-style enqueue-to-worker handoffs via `enqueues_job` semantic edges in both SPI and legacy extraction paths when queue and job names are literal, so queue-backed worker routes can survive slice selection without pretending the producer directly calls the worker.
