...
For runtime-generation prompts, those compact explain packs now also preserve NestJS/BullMQ-style enqueue-to-worker handoffs in their runtime-generation explain-pack relationships via `enqueues_job` semantic edges in both SPI and legacy extraction paths. This first pass is intentionally conservative: it only applies when job names are literal and the queue receiver is statically recognizable, but when it matches, queue-backed worker routes survive slice selection without pretending the producer directly calls the worker.
