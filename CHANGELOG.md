...
- **Queue-backed runtime paths keep their worker handoff**: runtime-generation retrieval now models NestJS/BullMQ-style enqueue-to-worker boundaries with `enqueues_job` semantic edges in both SPI and legacy extraction paths when queue and job names are literal, so compact explain packs can follow the semantic queue boundary instead of depending on direct call chains alone.
